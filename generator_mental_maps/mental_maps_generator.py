...
def generate_graph(main_topic, related_words):
    ...
    dot.render('2_graph', format='png', cleanup=True)
    return Image('2_graph.png')
...
def query_model(prompt, count):
    ...
    if count1 > 0:
      ...
      if answer_query_model is None:
        return query_model(prompt1, count1)
...
context = context_model(text, f"Вывод сделай в формате json. Ключей должно быть {len(listing)}. Значений должно быть {len(listing)}.")
...
if (listing_else_words):
  #запрос модели для перечислений непосредственно не связаны с главным словом
  context = context_model(text, f"Вывод сделай в формате json в виде ответов да/нет. Ключей должно быть {len(listing_else_words)}. Значений должно быть {len(listing_else_words)}.")
